...
На последнем месте работы в стартапе однял и настроил инфраструктуру проекта:

1) Настроил cicd через github actions, который запускал Ansible скрипт, который брал код с github и разворачивал его через docker на VPS сервере. Также прогон тестов и linters. Создал 3 окружения, dev, staging, production.

2) Подключил SSL и настроил автообновление. Настраивал DNS записи для разных поддоменов.

3) Поднял и настроил Nginx для раздачи фронтенда и проксирования запросов на бэкенд.

Имею опыт создания продуктов с нуля так и развитием существующего проекта.
...
### 1)
Добрый день, я бэкенд Nodejs разработчик с 4+ годами опыта.
На последнем проекте я занимаюсь разработкой масштабного туристического B2C продукта, с помощью модульного монолита с прицелом на переход на микросервисную архитектуру в случае необходимости.
...
На последнем месте работы в стартапе спроектировал архитектуру проекта с нуля.

Использовал C4 model, event storming. Выбрал за основу синхронную модульную архитектуру, что позволило взять лучшее из модульной и микросервисной архитектур.

На уровне модуля выбрал трехзвенную либо гексагональную архитектуру в зависимости от сложности доменной области.

В дальнейшем можно добавить eslint rule и ts-arch для поддержания выбранной архитектуры, а также настроить CI для автоматической проверки соблюдения архитектуры.

Всё это позволяет писать поддерживаемый, модульный, слабо связанный код, который в дальнейшем можно перевести на микросервисную архитектуру.
...
### 2. Почему ушли (уходите) с текущего места работы?
Ответы: разошлись пути, не было возможности для роста, произошли сокращения. Можно добавить лесть, например:
Я не собирался уходить, но ваша компания такая классная, которая делает крутые штуки, с крутой командой. Поэтому мне захотелось с вами поговорить.

### 3. Что говорить если не знаешь ответ?
Сначала нужно узнать что конкретно от вас хотят. Какие есть условия, какой должен быть результат. 
Самое главное начать рассуждать!
Пример: я собираюсь сделать вот это, это а потом вот это.

### 4. Ты работал с такой программой, стеком?
Да! Я справлюсь!
Пример: У меня есть такой то опыт, я могу применить в этой вашей технологии, программе, стеке.
Переносить свой опыт на стек который нужен им.
Всегда был весомой частью проекта.

### 5. Что для вас важно в компании / работе? (Почему хотите работать у нас?)
Для меня важны задачи, команда, продукт. Именно эта компания тебя потрясла, именно эта позиция и т.д. Деньги это тоже важно.

### 6. Собеседуетесь куда то еще?
Да, у меня 2 собеседования на этой неделе.

### 7. Какое ваше самое большое достижение?
Перед собесеодованием нужно смотреть на обязанности и описание вакансии, понять кого человека они ищут.
Выбрать самый сложный пункт и свои достижения подогнать под этот пункт.
Ваше достижение должно быть связано с работой которую от вас ждут.

### 8. Расскажите о себе.
В рассказе о себе нужно собрать все самое интересное в максимально короткой форме. 
Ответ должен быть релевантным к вакансии, емким, интригующим.
Т.е должна быть хорошая продажная структура в 1, 2 абзацах почему ты такой крутой специалист.

### 9. Какие у вас сильные и слабые стороны?
Сильные стороны должны крутиться вокруг профессии. Например: разбираюсь глубоко в технологии, которую другие плохо понимают, могу выстроить процесс.
Слабые стороны не должны относиться к профессии. Например: я тревожный человек, суеверный.

### 10. Есть ли у вас какие либо вопросы?
Отсутствие вопросов это признак:
- Некомпетентности;
- Незаинтересованности;
- безразличия;

Ответ: Надо задавать вопросы про цели компании, структуру, команды, процессы, испытательный срок.
В общем нужно узнать про компанию как можно больше и про ожидания от тебя.
Так ты создашь впечатление человека который выбирает компании среди нескольких других. 
Плюс дальше будет проще, т.к действительно будешь знать больше про компанию.
Нужно хотя бы 5-7 вопросов на собеседование.
Что нельзя спрашивать: простые вопросы по типу:
1) Что у вас за компания?
2) Что у вас за продукт?

Вопросы должны быть посложнее, например:
- Какое сейчас самое большое испытание у команды разработки? Какая самая большая проблема?
- Какие цели у компании, команды на ближайшие 3 месяца?

### 11. Расскажите о своих обязанностях на предыдущем месте работы?
Это не пересказ резюме. Нужно рассказать что то неочевидное, интересное. При ответе на этот вопрос ты продаешь себя.
Нужно рассказать о своих достижениях в формате:
Делал X, что привело к Y, что измеряется в Z.
Благодаря этому вопросу работадатель понимает нужен им такой специалист или нет.

### 12. Что ты ищешь от текущей позиции? (что тебе нужно)
Хорошо если ты  изначально узнал какого человека они ищут (кто их идеальный кандидат?) и под это можно подстроить ответ.
Например: говорят что можно перейти в team lead, ты не против и говоришь: "А, у вас можно перейти в team lead? Это как раз была моя карьерная цель." Нужно попадать в их ожидания.

### 13. Какие ваши зарплатные ожидания?
Не называй первым вилку. 
Если вынужден назвать вилку, скажи больше: Но если правильная компания, команда, проект и карьерный рост, то я готов чуть чуть подвинуться.
